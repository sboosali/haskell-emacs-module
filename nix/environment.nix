...
# : [Overlay]
# (where type Overlay = PackageSet -> PackageSet -> PackageSet 
...
# (where type PackageSet = {<PackageName>: <Derivation>, ...})

, nixpkgs ? nixpkgsWith { inherit overlays; }
# : PackageSet 
# e.g. nixpkgs ? import <nixpkgs> {}

, compiler ? null
# : Maybe String 
# `null` means: use the default. (i.e. `haskellPackages` not `haskell.packages.${compiler}`)
# e.g. "ghc7103", "ghc802", "ghc822", "ghc841", "ghcHEAD", "ghc7103Binary", "ghc821Binary", "ghcjs", "ghcjsHEAD", "integer-simple.<compiler>
...
# : Boolean
# `true` means: 
# * add development dependencies like Version Control stuff and editor stuff; 
# * and maybe modify the package itself if it supports a development flag; i.e. "-fdevelopment", or "-fdevelop", or "-fdevel"; (invalid cabal flags are silently ignored, i.e. are not failures). 
